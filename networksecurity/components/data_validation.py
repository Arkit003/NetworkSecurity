...
class DataValidation:
    ...
    @staticmethod # behaves like a normal function but lives in class namespace, DataValidation.read_data
    ...
    def validating_no_of_columns(self,dataframe:pd.DataFrame)-> bool:
        try:
    ...
    def checking_numerical_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            num_cols=dataframe.select_dtypes(include='number').columns.tolist()
            if len(num_cols) != self._schema_config:
                return True
            return False
        except Exception as e:
            raise CustomException(e,sys)
    ...
    def detect_dataset_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame,threshold:float=0.05)->bool:
        try:
            ...
            for column in base_df.columns:
                ...
                #?? can we make dir with file name ?? 
    ...
    def initiate_data_validation(self) ->DataValidationArtifact :
        try:
            ...
            status=self.validating_no_of_columns(train_df)
            if not status:
                error_message_train=f"Train dataframe does not contain all columns.\n"
            status=self.validating_no_of_columns(test_df)
            if not status:
                error_message_test=f"Test dataframe does not contain all columns.\n"
